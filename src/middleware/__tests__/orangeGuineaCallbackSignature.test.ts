...
import { verifyOrangeGuineaCallbackSignature } from "../orangeGuineaCallbackSignature";
...
function makeReq(
  ...
  return {
    ...
    body: {},
    method: "POST",
    originalUrl: "/api/orange-guinea/callback",
    url: "/api/orange-guinea/callback",
    ...overrides,
...
beforeEach(() => {
  ...
  mockGetConfigValue.mockImplementation((key: string) => {
    if (key === "providers.orangeGuinea.callbackSecret") return SECRET;
...
describe("verifyOrangeGuineaCallbackSignature", () => {
  describe("secret not configured", () => {
    it("returns 500 and logs anomaly when secret is missing", async () => {
      mockGetConfigValue.mockImplementation((key: string) => {
        if (key === "providers.orangeGuinea.callbackSecret") return "";
        return undefined;
      });
      ...
      await verifyOrangeGuineaCallbackSignature(req, res, next);

      expect(res.status).toHaveBeenCalledWith(500);
      expect(res.json).toHaveBeenCalledWith({
        error: "Orange Guinea callback verification not configured",
      });
      expect(next).not.toHaveBeenCalled();
      expect(mockLogSecurityAnomaly).toHaveBeenCalledWith(
        expect.objectContaining({
          reason: "orange_guinea_callback_secret_not_configured",
        }),
      );
    });
  });
  ...
  describe("signature header missing", () => {
    it("throws 401 and logs anomaly when no signature header is present", async () => {
      ...
      await expect(
        verifyOrangeGuineaCallbackSignature(req, makeRes(), next),
      ).rejects.toMatchObject({ code: "UNAUTHORIZED" });

      expect(next).not.toHaveBeenCalled();
      expect(mockLogSecurityAnomaly).toHaveBeenCalledWith(
        expect.objectContaining({
          reason: "orange_guinea_callback_signature_missing",
        }),
      );
    });
  });
  ...
  describe("valid signatures", () => {
    it("calls next() for a valid base64 HMAC signature using rawBody", async () => {
      ...
      await verifyOrangeGuineaCallbackSignature(req, makeRes(), next);
    ...
    it("accepts the signature via the alt header x-orange-signature", async () => {
      mockGetConfigValue.mockImplementation((key: string) => {
        if (key === "providers.orangeGuinea.callbackSecret") return SECRET;
        if (key === "providers.orangeGuinea.callbackSignatureHeader")
  ...
  describe("invalid signatures", () => {
    it("throws 401 for a tampered payload", async () => {
      ...
      expect(mockLogSecurityAnomaly).toHaveBeenCalledWith(
        expect.objectContaining({
          reason: "orange_guinea_callback_signature_invalid",
        }),
      );
    });
