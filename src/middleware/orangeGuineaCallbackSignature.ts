...
function getSignatureHeaderName(): string {
  const configured = String(
    getConfigValue("providers.orangeGuinea.callbackSignatureHeader") ?? "",
  )
    .trim()
    .toLowerCase();
...
export async function verifyOrangeGuineaCallbackSignature(
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> {
  ...
  try {
    ...
    logSecurityAnomaly({
      ...
      method: req.method,
      ip: getCurrentRequestIp(req),
      reason: "orange_guinea_callback_signature_error",
      provider: "orange_guinea",
      headerPresent: true,
    });
    throw createError(ERROR_CODES.UNAUTHORIZED, "Unauthorized callback", {
