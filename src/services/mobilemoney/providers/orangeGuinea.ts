...
const DEFAULT_PAYMENT_PATH = "/orange-money-webpay/gn/v1/payments/collect";
const DEFAULT_PAYOUT_PATH = "/orange-money-webpay/gn/v1/payments/disburse";
const DEFAULT_STATUS_PATH = "/orange-money-webpay/gn/v1/payments";
...
export class OrangeGuineaProvider extends BaseProvider {
  ...
  async sendBatchPayout(items: BatchPayoutItem[]): Promise<{
    ...
    logger.info(
      ...
      "OrangeGuinea: Starting batch payout",
    );
    ...
    try {
      ...
      logger.info(
        ...
        "OrangeGuinea: Batch payout completed",
      );
      ...
      logger.error(
        ...
        "OrangeGuinea: Batch payout failed",
      );
      return {
        success: false,
        results: items.map((item) => ({
  ...
  async getOperationalBalance(): Promise<{
    ...
    try {
      ...
      const response = await this.sendRequest({
        method: "GET",
        url: `${this.baseUrl}/orange-money-webpay/gn/v1/account/balance`,
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  private async executeOperation(
    ...
    log.info(
      ...
      "OrangeGuinea: Executing operation",
    );
    const startTime = Date.now();
    ...
    try {
      ...
      log.info(
        ...
        "OrangeGuinea: Operation completed",
      );
  ...
  async getAccessToken(forceRefresh = false): Promise<string> {
    ...
    this.authPromise = (async () => {
      try {
        ...
        if (response.status < 200 || response.status >= 300) {
          throw new Error(
            `OrangeGuinea auth failed with status ${response.status}`,
          );
        }
        ...
        if (!data.access_token) {
          throw new Error("OrangeGuinea auth did not return access_token");
        }
  ...
  private schedulePrefetch(ttlMs: number, isRetry = false): void {
    ...
    this.prefetchTimer = setTimeout(async () => {
      if (this.destroyed) return;
      try {
        logger.info("OrangeGuinea: Pre-fetching access token");
        await this.getAccessToken(true);
      } catch (error: any) {
        if (this.destroyed) return;
        logger.error(
          { error: error.message },
          "OrangeGuinea: Token pre-fetch failed, retrying",
        );
  ...
  private createReference(operation: "payment" | "payout"): string {
    return `ORANGE-GN-${operation.toUpperCase()}-${this.clock()}-${randomUUID().slice(0, 8)}`;
  }
