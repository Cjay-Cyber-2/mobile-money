...
export class OrangeMadagascarProvider extends BaseProvider {
  ...
  constructor() {
    super({
      apiKey: process.env.ORANGE_MADAGASCAR_API_KEY || "",
      apiSecret: process.env.ORANGE_MADAGASCAR_API_SECRET || "",
      baseUrl: process.env.ORANGE_MADAGASCAR_BASE_URL ?? DEFAULT_BASE_URL,
      timeoutMs: Number(process.env.ORANGE_MADAGASCAR_TIMEOUT_MS || DEFAULT_TIMEOUT_MS),
    });
    this.clock = Date.now;
    ...
    this.callbackSignatureHeader = process.env.ORANGE_MADAGASCAR_CALLBACK_SIGNATURE_HEADER ?? "X-Oapi-Signature";
